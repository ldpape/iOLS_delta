...
{depvar} [{it:indepvars}]
{if}  {cmd:,} delta(#) endog({it:endogvars}) instr({it:instruments}) [{help i2SLS_delta##options:options}] {p_end}
...
{phang2}{cmd:. xi: eststo reg1: i2SLS_delta trips cbd ptn  i.weekend , delta(1) robust endog(tcost) instr(worker)}{p_end}
{phang2}{cmd:. xi: bootstrap lambda_stat=e(lambda) ,    reps(50):  i2SLS_delta_test trips cbd ptn i.weekend  , delta(1) endog(tcost) instr(worker) nonparametric  }{p_end}
