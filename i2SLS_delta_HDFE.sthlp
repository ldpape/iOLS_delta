...
{viewerjumpto "Syntax" "i2SLS_delta_hdfe##syntax"}{...}
{viewerjumpto "Description" "i2SLS_delta_hdfe##description"}{...}
{viewerjumpto "Citation" "i2SLS_delta_hdfe##citation"}{...}
{viewerjumpto "Authors" "i2SLS_delta_hdfe##contact"}{...}
{viewerjumpto "Examples" "i2SLS_delta_hdfe##examples"}{...}
{viewerjumpto "Description" "i2SLS_delta_hdfe##Testing"}{...}
{viewerjumpto "Stored results" "i2SLS_delta_hdfe##results"}{...}

{title:Title}

{p2colset 5 18 20 2}{...}
{p2col :{cmd:i2SLS_delta_hdfe} {hline 2}} Iterated Two Stage Least Squares (i2SLS) with delta and High-Dimensional Fixed Effects {p_end}

{p2colreset}{...}

{pstd}{cmd:Introduction} This program implements iterated Two Stage Least Squares with delta, as described by {browse "https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3444996":Bellego, Benatia, and Pape (2021)}. {cmd: i2SLS_delta_hdfe} is a solution to the problem of the log of zero with endogenous covariates.  This method relies on running the "ivreg2" function iteratively. This provides the reader with the final IV estimates and allows the use the post-estimation commands available under regress (using Y_tilde = log(Y + delta*exp(xb))) as a dependent variable. Delta allows the user to assess several moment conditions to assess the robustness of the parameter estimates to moment specification. The program {cmd:i2SLS_delta_hdfe_test} provide a test to assess how well each of these moments, which have implications in terms of the patter n of zeros in the data, compare to the pattern of zeros observed in the data. 

{pstd}{cmd:Fixed-Effects:} This package takes a within-transformation to difference out high-dimensional fixed effects. To do so, it relies on the HDFE package developed by {browse "http://scorreia.com/research/hdfe.pdf": Sergio Correia (2017)}. In turn, the obtained standard errors will slightly differ from those obtained running the same model with i2SLS_delta_hdfe. 
...
{p 8 15 2} {cmd:i2SLS_delta_hdfe}
{depvar} [{indepvars}]
{it:if}  {cmd:,} delta(#) endog({it:endogvars}) instr({it:instruments}) absorb({it:fixed-effects}) [{help i2SLS_delta_hdfe##options:options}] {p_end}
...
{syntab: Convergence}
{synopt:{opt limit}{cmd:(}{help i2SLS_delta_hdfe##limit:limit}{cmd:)}} Choose convergence criteria in terms of mean squared difference between two set of paramter estimates between two iterations. Set to 1e-8 if unspecified. {p_end}
{synopt:{opt maximum}{cmd:(}{help i2SLS_delta_hdfe##maximum:maximum}{cmd:)}} Maximum number of iterations. Set to 10,000 if unspecified. {p_end}

{marker Post-Estimation}{...}
{title:Post-Estimation}

{pstd} This program generates three variables : {cmd:i2SLS_delta_hdfe_xb} which calculates the linear index for the final sample, {cmd:i2SLS_delta_hdfe_U} which provides the un-transformed residual U_i, and {cmd:i2SLS_delta_hdfe_fe} which calculates the sum of individual fixed-effects for each observations. 
...
{pstd}
{cmd:i2SLS_delta_hdfe} stores the following in {cmd:e()}:
...
{synopt:{cmd:i2SLS_delta_hdfe_U}} un-transformed residual U_hat from Y = exp(Xb+fe)*U {p_end}
{synopt:{cmd:i2SLS_delta_hdfe_xb}} estimated linear index Xb {p_end}
{synopt:{cmd:i2SLS_delta_hdfe_fe}} estimated fixed effect fe {p_end}
