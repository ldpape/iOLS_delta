...
{p2colset 5 18 20 2}{...}
{p2col :{cmd:iOLS_delta_HDFE_test} {hline 2}} External Validity Test for iOLS_delta_HDFE {p_end}

{p2colreset}{...}

{pstd}{cmd:Introduction} The program {cmd:iOLS_delta_HDFE_test} provide a test to assess the externality of iOLS_delta with respect to how well it fits the pattern of zeros observed in the data. This test is based on the implied proportionality condition which relates the conditional probability of a zero given X in the data to the residuals of iOLS. This program first estimates iOLS_delta and a parametric (logit) or semi-nonparametric conditional probability model (multivariate nearest-neighbor smoother). It then assesses if the residuals from iOLS_delta are proportional to the predicted probabilities of the latter model, as described by {browse "https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3444996":Bellego, Benatia, and Pape (2021)}.  It provides the reader with a statistic, lambda_stat, which should be near 1 if (a) the probability model is a consistent estimate of Pr(Y>0|X) and (b) iOLS_delta captures well the pattern of zeros in the data. 
...
{p 8 15 2} {cmd:iOLS_delta_HDFE_test}
{depvar} [{indepvars}]
{if}  {cmd:,} delta(#) absorb({it:fixed-effects}) [{help iOLS_delta##options:options}] {p_end}
...
{phang2}{cmd:. xi: eststo reg1: iOLS_delta_HDFE trips cbd ptn worker  tcost, delta(1) absorb(weekend) robust }{p_end}
{phang2}{cmd:. xi: bootstrap lambda_stat=e(lambda) ,    reps(50):  iOLS_delta_HDFE_test trips cbd ptn worker  tcost , delta(1) absorb(weekend) logit  }{p_end}
...
{pstd}
{cmd:iOLS_delta_HDFE_test} stores the following in {cmd:e()}:

{synoptset 24 tabbed}{...}
{syntab:Scalars}
{synopt:{cmd:e(lambda)}} Lambda statistic {p_end}
{syntab:Matrices}
{synopt:{cmd:e(beta_hat_iols)}} beta_hat from running iOLS_delta{p_end}
{synopt:{cmd:e(var_beta_hat_iols)}} variance-covariance matrix of beta_hat from running iOLS_delta{p_end}
