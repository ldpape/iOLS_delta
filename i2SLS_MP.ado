...
* 20/04 :  quiet the collinearity + SHOW option
...
syntax varlist [if] [in] [aweight pweight fweight iweight] [, DELta(real 1) from(name) endog(varlist) instr(varlist) LIMit(real 1e-8)   MAXimum(real 10000) SHOW Robust CLuster(string)]           
...
quietly: ivreg2 `y_tild' `exogenous' (`endog' = `instr') [`weight'`exp'] if `touse', `option' 
	...
	*** Stocker les resultats dans une matrice
    ...
    ereturn post beta_final Sigma_tild , obs(`e(N)') depname(`depvar') esample(`touse')  dof(`dof') 
	 cap drop i2SLS_MP_xb_hat
	cap drop i2SLS_MP_error
    	mata: st_store(., st_addvar("double", "i2SLS_MP_error"), "_COPY", ui)
    	mata: st_store(., st_addvar("double", "i2SLS_MP_xb_hat"),"_COPY", xb_hat)
		cap drop _COPY
